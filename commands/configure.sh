...
if test ! -e "profiles.conf"; then
  echo "You need to manually fetch profiles.conf from the Plastic user-local files folder (typically C:\Users\<username>\AppData\Local\plastic4) and place it in your current dir, before running this script"
  exit 1
fi
...
grep -E -m 1 -o "<SecurityConfig>(.*)</SecurityConfig>" profiles.conf | sed -e 's,.*<SecurityConfig>\([^<]*\)</SecurityConfig>.*,\1,g' >> authentication.conf
